...
def test_invalid_image_scheme_is_rejected():
    country = json.loads((ROOT / "config" / "countries.json").read_text())[0]
    date_key = next(iter(json.loads((ROOT / "data" / "manifest.json").read_text())["countries"]["US"]["dates"]))
    payload = json.loads((ROOT / "data" / "US" / f"{date_key}.json").read_text())
